...
save='''  const saveRepair = useCallback((r: RepairJob) => {
    if (!user) return;
...
rep='''  const deleteRepair = useCallback((id: string) => {
...
'''
s=s[:a]+rep+s[b:];s=s.replace('    saveRepair, updateRepairStatus, deleteRepair,','    saveRepair, updateRepairStatus, deleteRepair, saveWarrantyClaim,');store.write_text(s)
