...
const STATUS_LABEL: Record<RepairStatus, string> = {
  received: 'Received', diagnosed: 'Diagnosed', waiting_parts: 'Waiting parts', in_repair: 'In repair',
  ready: 'Ready', delivered: 'Delivered', cancelled: 'Cancelled',
};

const TONE: Record<string, string> = {
  received: 'sky', diagnosed: 'violet', waiting_parts: 'amber', in_repair: 'blue',
  ready: 'emerald', delivered: 'slate', cancelled: 'rose',
};
...
export default function Repairs() {
  ...
  const availableProducts = state.products.filter(p => p.active && !p.isService && p.stock > 0);
  ...
  const rows = useMemo(() => {
    ...
    return jobs.filter(j => {
      ...
      return j.jobNo.toLowerCase().includes(query) || j.customerName.toLowerCase().includes(query) ||
        j.deviceModel.toLowerCase().includes(query) || j.deviceBrand.toLowerCase().includes(query) ||
        (j.imei || '').toLowerCase().includes(query) || (j.serial || '').toLowerCase().includes(query) || j.fault.toLowerCase().includes(query);
    });
  }, [jobs, q, filter]);
  ...
  const openNew = () => {
    setEditing({ ...emptyJob(), by: user?.name || 'Staff', warrantyDays: state.settings.repairWarrantyDays ?? 30 });
    setIsNew(true); setPartProductId(''); setPartQty(1);
  };

  const addPart = () => {
    if (!editing || !partProductId || !Number.isInteger(partQty) || partQty <= 0) return;
    const product = state.products.find(p => p.id === partProductId);
    if (!product || !product.active || product.isService || partQty > product.stock) return;
    const existing = editing.parts.find(p => p.productId === product.id);
    const existingQty = existing?.qty || 0;
    if (existingQty + partQty > product.stock) return;
    const parts = existing
      ? editing.parts.map(p => p.productId === product.id ? { ...p, qty: p.qty + partQty } : p)
      : [...editing.parts, { productId: product.id, name: product.name, qty: partQty, cost: product.cost } as RepairPart];
  ...
  const removePart = (index: number) => {
    ...
    setEditing({ ...editing, parts: editing.parts.filter((_, i) => i !== index) });
  };
  ...
  const save = () => {
    ...
    if (!financialValues.every(Number.isFinite) || editing.laborCost < 0 || (editing.advancePaid || 0) < 0 || (editing.warrantyDays || 0) < 0) return;
    if (editing.parts.some(p => !p.productId || !state.products.some(x => x.id === p.productId) || !Number.isInteger(p.qty) || p.qty <= 0 || !Number.isFinite(p.cost) || p.cost < 0)) return;
    // Existing jobs keep their original parts here. This prevents the current
  ...
  return (
    <div>
      <PageHeading chip="Service" chipTone="emerald" title="Repairs & Service Jobs"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-4">
        {PIPELINE.map(s => <button key={s} type="button" onClick={() => setFilter(s)} className={`card p-3 text-left transition ${filter === s ? 'ring-2 ring-violet-500/50' : ''}`}>
          <div className="text-[10px] font-bold uppercase tracking-wider text-faint">{STATUS_LABEL[s]}</div><div className="text-xl font-extrabold text-ink num mt-0.5">{counts[s] || 0}</div>
        </button>)}
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="p-4 border-b border-line flex flex-wrap gap-3 items-center">
          ...
          <select className="input w-44" value={filter} onChange={e => setFilter(e.target.value)}><option value="active">Active jobs</option><option value="all">All jobs</option>{PIPELINE.map(s => <option key={s} value={s}>{STATUS_LABEL[s]}</option>)}<option value="cancelled">Cancelled</option></select>
        </div>
        {rows.length === 0 ? <EmptyState icon={<Wrench size={26} />} title="No repair jobs" sub="Create a job card when a device is received" /> : <div className="divide-y divide-line">
          {rows.map(j => <div key={j.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-3 hover:bg-raised/30">
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title={isNew ? 'New repair job' : editing?.jobNo || 'Job'} sub={editing ? `${editing.deviceBrand} ${editing.deviceModel}` : ''}>
        {editing && <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-3 gap-2"><Field label="Type"><select className="input" value={editing.deviceType} onChange={e => setEditing({ ...editing, deviceType: e.target.value })}>{['Phone', 'Laptop', 'Desktop', 'Tablet', 'Other'].map(t => <option key={t}>{t}</option>)}</select></Field>
            ...
            <Field label="Model"><input className="input" value={editing.deviceModel} onChange={e => setEditing({ ...editing, deviceModel: e.target.value })} /></Field></div>
