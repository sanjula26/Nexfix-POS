...
export default function Units() {
  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">{rows.length===0?<EmptyState icon={<Smartphone size={28}/>} title="No units found" sub="Add a unit or change the search filter."/>:<div className="divide-y divide-slate-100">{rows.map(u=>{const p=products.find(x=>x.id===u.productId);return <div key={u.id} className="flex items-center justify-between gap-4 px-4 py-3"><div><div className="font-medium text-slate-900">{p?.name||'Unknown product'}</div><div className="text-xs text-slate-500">{u.imei||u.serial||'No IMEI/serial'} · Added {fmtDate(u.createdAt)}</div></div><div className="flex items-center gap-3"><Badge tone={STATUS_TONE[u.status]}>{STATUS_LABEL[u.status]}</Badge>{can('act:manageStock')&&<button className="btn-ghost" onClick={()=>{setEditing(u);setIsNew(false)}} aria-label="Edit unit"><Cpu size={16}/></button>}{can('act:manageStock')&&u.status==='in_stock'&&<button className="btn-ghost text-rose-600" onClick={()=>deleteUnit(u.id)} aria-label="Delete unit"><Trash2 size={16}/></button>}</div></div>})}</div>}</div>
