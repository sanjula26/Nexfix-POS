...
export default function Units() {
  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">{rows.length===0?<EmptyState icon={<Smartphone size={28}/>} title="No units found" sub="Add a unit or change the search filter."/>:<div className="divide-y divide-slate-100">{rows.map(u=>{const p=products.find(x=>x.id===u.productId);return <div key={u.id} className="flex items-center justify-between gap-4 px-4 py-3"><div><div className="font-medium text-slate-900">{p?.name||'Unknown product'}</div><div className="text-xs text-slate-500">{u.imei||u.serial||'No IMEI/serial'} · Added {fmtDate(u.createdAt)}</div></div><div className="flex items-center gap-3"><Badge tone={STATUS_TONE[u.status]}>{STATUS_LABEL[u.status]}</Badge>{can('act:manageStock')&&<button className="btn-ghost" onClick={()=>{setEditing(u);setIsNew(false)}} aria-label="Edit unit"><Cpu size={16}/></button>}{can('act:manageStock')&&can('act:deleteRecords')&&u.status==='in_stock'&&<button className="btn-ghost text-rose-600" onClick={()=>deleteUnit(u.id)} aria-label="Delete unit"><Trash2 size={16}/></button>}</div></div>})}</div>}</div>
...
function UnitEditor({value,products,units,isNew,onSave,onClose}:{value:InventoryUnit|null;products:any[];units:InventoryUnit[];isNew:boolean;onSave:(u:InventoryUnit)=>void;onClose:()=>void}){
 ...
 return <div className="space-y-4"><Field label="Product"><select className="input" value={productId} onChange={e=>{setProductId(e.target.value);setError('')}} disabled={!isNew}>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><div className="grid grid-cols-1 gap-4 md:grid-cols-2">
  ...
  <Field label={product?.trackSerial ? 'Serial *' : 'Serial'} hint={product?.trackSerial ? 'Required for this product' : 'Not required'}>
    <input className={`input ${product?.trackSerial && !serial.trim() ? 'border-amber-500/40' : ''}`} value={serial} onChange={e=>{setSerial(e.target.value);setError('')}} placeholder={product?.trackSerial ? 'Enter serial number' : 'Optional'} />
  </Field>
</div>
...
</div><Field label="Status"><select className="input" value={status} onChange={e=>setStatus(e.target.value as UnitStatus)} disabled={isNew||!!value}><option value="in_stock">In stock</option>{(Object.keys(STATUS_LABEL) as UnitStatus[]).filter(s=>s!=='in_stock').map(s=><option key={s} value={s}>{STATUS_LABEL[s]}</option>)}</select>{isNew?<div className="mt-1 text-xs text-slate-500">New units use an existing received stock unit when one is waiting for its IMEI/serial; otherwise stock increases by 1.</div>:<div className="mt-1 text-xs text-slate-500">Unit status is changed automatically by sales, returns, repairs and stock workflows.</div>}</Field><Field label="Note"><textarea className="input min-h-24" value={note} onChange={e=>setNote(e.target.value)}/></Field>{error&&<p role="alert" className="text-sm font-medium text-rose-600">{error}</p>}<div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end"><button type="button" className="btn btn-soft w-full !px-5 !py-2.5 sm:w-auto" onClick={onClose}>Cancel</button><button type="button" className="btn btn-primary w-full !px-5 !py-2.5 sm:w-auto" onClick={submit}>Save</button></div></div>;
}
