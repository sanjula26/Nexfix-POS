...
export default function Exchanges() {
  ...
  /** Resolve local sale lines to cloud sale_item UUIDs. Duplicate untracked lines are interchangeable;
   * tracked lines are matched by their exact returned unit ids. */
  const resolveCloudLines = async (shopId: string, sale: Sale, selectedItems: Array<{ itemIdx: number; qty: number }>) => {
    if (!supabase) throw new Error('Cloud is not configured');
    const { data, error: queryError } = await supabase
      .from('sale_items')
      .select('id,product_id,qty,price,discount,unit_ids')
      .eq('sale_id', sale.id);
    ...
    return selectedItems.map(({ itemIdx, qty }) => {
      ...
      const candidates = data.filter(row => {
        ...
        return [...localUnits].some(id => cloudUnits.has(id));
      });
      const row = candidates[0];
      if (!row) throw new Error(`Cloud sale item could not be matched for ${local.name}`);
      used.add(row.id);
      const unitIds = local.unitIds?.slice(0, qty);
    ...
    <div>
      ...
      <div className="card p-6 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2.5">
          <div className="relative flex-1"><ScanSearch size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-faint" /><input className="input pl-9 num" placeholder="Enter bill number (e.g. NFX-20260813-1042)" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && search()} /></div>
          <button className="btn btn-primary" onClick={search} disabled={!query.trim()}><ScanSearch size={15} /> Search Bill</button>
        </div>
        ...
        {bill && (
          <div className="mt-5 space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              {bill.status !== 'completed' ? <Badge tone="rose">ALREADY {bill.status.toUpperCase()}</Badge> : withinPolicy ? <Badge tone="emerald"><CheckCircle2 size={11} /> WITHIN {state.settings.exchangeDays}-DAY POLICY</Badge> : <Badge tone="rose"><ShieldAlert size={11} /> POLICY EXPIRED</Badge>}
            </div>
            <div className="rounded-xl border border-line overflow-hidden">
              <table className="w-full"><thead><tr><th className="th w-10"></th><th className="th">Item</th><th className="th">Qty</th><th className="th">Price</th><th className="th !text-right">Amount</th></tr></thead>
                <tbody>{bill.items.map((it, i) => <tr key={i} className={`cursor-pointer transition-colors ${selected.includes(i) ? 'bg-violet-500/[0.07]' : 'hover:bg-raised/40'}`} onClick={() => toggle(i)}>
                  <td className="td"><span className={`w-[18px] h-[18px] rounded-[5px] border flex items-center justify-center transition-all ${selected.includes(i) ? 'bg-violet-600 border-violet-600 text-white' : 'border-line bg-surface'}`}>{selected.includes(i) && <CheckCircle2 size={12} strokeWidth={3} />}</span></td>
                  <td className="td font-medium text-[13px]">{it.name}</td><td className="td num">{it.qty}</td><td className="td num">{fmtRs(it.price)}</td>
                  <td className="td num font-semibold text-right">{selected.includes(i) ? <input type="number" min={1} max={it.qty} className="input w-20 !py-1 text-right" value={returnQty[i] || ''} onClick={ev => ev.stopPropagation()} onChange={ev => setReturnQty(q => ({ ...q, [i]: Math.max(0, Math.min(it.qty, Number(ev.target.value) || 0)) }))} aria-label={`Return quantity for ${it.name}`} /> : it.qty}</td>
                </tr>)}</tbody>
              </table>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div><span className="block text-[11px] font-bold tracking-wider uppercase text-sub mb-1.5">Reason</span><select className="input" value={reason} onChange={e => setReason(e.target.value)}>{['Defective item', 'Wrong model / color', 'Customer changed mind', 'Not working as expected', 'Damaged on arrival'].map(r => <option key={r}>{r}</option>)}</select></div>
              <div><span className="block text-[11px] font-bold tracking-wider uppercase text-sub mb-1.5">Resolution</span><div className="grid grid-cols-2 gap-2"><button className={`btn ${mode === 'replace' ? 'btn-primary' : 'btn-soft'}`} onClick={() => setMode('replace')}><Repeat2 size={14} /> Replace item</button><button className={`btn ${mode === 'refund' ? 'btn-primary' : 'btn-soft'}`} onClick={() => setMode('refund')} disabled={!can('act:refund')}><RotateCcw size={14} /> Refund cash</button></div></div>
            </div>
      ...
      <Modal open={confirm} onClose={() => !processing && setConfirm(false)} title={mode === 'refund' ? 'Confirm return' : 'Confirm exchange'} sub={bill?.billNo}>
        <p className="text-sm text-sub">{mode === 'refund' ? <>Return <b className="num text-ink">{fmtRs(selectedSum)}</b> cash to the customer and record the return atomically.</> : <>Restock the selected returned item(s) and record the return atomically. The replacement item must be processed as a separate sale so its stock and revenue are recorded.</>}</p>
        <div className="flex gap-2.5 mt-5"><button className="btn btn-primary flex-1" onClick={doProcess} disabled={processing}><CheckCircle2 size={15} /> {processing ? 'Processing…' : 'Confirm'}</button><button className="btn btn-soft flex-1" onClick={() => setConfirm(false)} disabled={processing}>Cancel</button></div>
      </Modal>
    </div>
  );
}
