...
export default function MobileDashboard() {
  ...
  const profit = rangeSales.reduce((sum, s) => sum + s.profit, 0);
  ...
  return <main className="min-h-screen bg-slate-50 p-4 pb-8 sm:p-6">
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      <section className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
        <label htmlFor="mobile-period" className="mb-1 block text-xs font-medium text-slate-500">Performance period</label>
        <select id="mobile-period" value={period} onChange={e => setPeriod(Number(e.target.value) as 1 | 7 | 30)} className="w-full rounded-xl border-0 bg-slate-50 px-3 py-2.5 text-sm font-semibold text-slate-800 ring-1 ring-slate-200 focus:ring-2 focus:ring-violet-500">
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[['Sales', money(revenue)], ...(canViewProfit ? [['Profit', money(profit)] as const] : []), ['Expenses', money(expensesInRange)], ['Low stock', String(lowStock)]].map(([label, value]) => <article key={label} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-lg font-bold text-slate-900">{value}</p></article>)}
      </section>
      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200"><h2 className="font-semibold text-slate-900">Recent bills</h2><div className="mt-3 space-y-2">{rangeSales.slice(-8).reverse().map(s => <div key={s.id} className="flex justify-between border-b border-slate-100 py-2 text-sm"><span>{s.billNo}</span><span className="font-medium">{money(s.total)}</span></div>)}{!rangeSales.length && <p className="text-sm text-slate-500">No bills in this period.</p>}</div></section>
    </div>
  </main>;
}
