...
export default function MobileDashboard() {
  ...
  const rangeSales = useMemo(() => sales.filter(s => {
    ...
    return day >= rangeStart && day <= today && s.status === 'completed';
  ...
  const filteredSales = useMemo(() => effectiveMachineFilter === 'all'
    ? rangeSales
    : rangeSales.filter(s => (s.machineId || 'legacy') === effectiveMachineFilter), [rangeSales, effectiveMachineFilter]);
  ...
  return <main className="min-h-screen bg-slate-50 p-4 pb-8 sm:p-6">
    <div className="mx-auto max-w-5xl space-y-4">
      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200"><h2 className="font-semibold text-slate-900">Recent bills</h2><div className="mt-3 space-y-2">{filteredSales.slice(-8).reverse().map(s => <div key={s.id} className="flex justify-between border-b border-slate-100 py-2 text-sm"><span>{s.billNo}</span><span className="font-medium">{money(s.total)}</span></div>)}{!filteredSales.length && <p className="text-sm text-slate-500">No bills in this period.</p>}</div></section>
    </div>
  </main>;
}
