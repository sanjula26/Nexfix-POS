...
function printGRN(grn: Purchase, shopName: string) {
  const w = window.open('', '_blank', 'width=760,height=900');
  if (!w) return;
  const rows = grn.items.map(i => `<tr><td>${i.name}</td><td>${i.qty}</td><td>Rs. ${i.cost.toLocaleString()}</td><td>Rs. ${(i.qty * i.cost).toLocaleString()}</td></tr>`).join('');
  w.document.write(`<!doctype html><html><head><title>${grn.poNo}</title><style>body{font-family:Arial,sans-serif;padding:28px;color:#111}h1{margin:0 0 4px}p{margin:5px 0;color:#555}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border-bottom:1px solid #ddd;padding:9px;text-align:left}th:last-child,td:last-child{text-align:right}.total{text-align:right;font-size:18px;font-weight:700;margin-top:18px}</style></head><body><h1>${shopName}</h1><h2>Goods Received Note</h2><p><b>${grn.poNo}</b> · ${fmtDate(grn.date)}</p><p>Supplier: ${grn.supplierName}</p>${grn.supplierInvoiceNo ? `<p>Supplier Invoice: ${grn.supplierInvoiceNo}</p>` : ''}<p>Status: ${grn.status === 'received' ? 'PROCESSED' : 'DRAFT'}</p><table><thead><tr><th>Item</th><th>Qty</th><th>Cost</th><th>Total</th></tr></thead><tbody>${rows}</tbody></table><div class="total">Total: Rs. ${grn.total.toLocaleString()}</div>${grn.notes ? `<p>Notes: ${grn.notes}</p>` : ''}</body></html>`);
