...
export default function GRN() {
  ...
  const buildItems = (): PurchaseItem[] => rows.filter(r => r.productId && r.qty > 0).map(r => {
  ...
  const validateForm = (items: PurchaseItem[]) => {
    ...
    if (items.some(i => !Number.isInteger(i.qty) || i.qty <= 0)) return 'Quantity must be a positive whole number.';
  ...
  const saveDraft = () => {
    ...
    if (editingId) {
      const ok = updateGRNDraft(editingId, payload);
      if (!ok) { setFormError('Unable to update this GRN draft. Check your permissions and confirm the draft still exists.'); return; }
    } else {
      const created = saveGRNDraft(payload);
      if (!created) { setFormError('Unable to save the GRN draft. Check your permissions and try again.'); return; }
    }
    setView('list');
  };
  ...
  const askProcess = () => {
    ...
    if (editingId) {
      const ok = updateGRNDraft(editingId, { supplierId: supplier.id, supplierName: supplier.name, supplierInvoiceNo: invoiceNo.trim() || undefined, notes: notes.trim() || undefined, items, total: items.reduce((sum, i) => sum + i.qty * i.cost, 0) });
      if (!ok) { setFormError('Unable to update this GRN draft. Check your permissions and confirm the draft still exists.'); return; }
  ...
  const process = () => {
    ...
    if (!validatePurchaseUnitIdentifiers(purchase, state.products, state.units || [])) {
  ...
  return <div>
    ...
    {view === 'form' ? <div className="card p-5"><div className="flex items-center justify-between mb-5"><div><h2 className="font-bold text-ink">{editingId ? 'Edit GRN draft' : formMode === 'return' ? 'New Supplier Return' : 'New GRN'}</h2><p className="text-xs text-sub mt-1">{formMode === 'return' ? 'Return received items to the supplier and deduct stock automatically.' : 'Stock is updated only when the GRN is processed.'}</p></div><button className="btn btn-soft" onClick={() => setView('list')}><X size={14} /> Close</button></div>{!editingId && <div className="flex gap-2 p-1 rounded-xl bg-raised border border-line mb-5"><button className={`flex-1 btn ${formMode === 'receive' ? 'btn-primary' : 'btn-soft'}`} onClick={() => { setFormError(''); setFormMode('receive'); }}><PackageCheck size={14} /> Receive Stock</button><button className={`flex-1 btn ${formMode === 'return' ? 'bg-rose-600 text-white' : 'btn-soft'}`} onClick={() => { setFormError(''); setFormMode('return'); setReturnPurchase(null); setReturnLines([]); setReturnReason(''); setReturnGrnId(''); }}><Trash2 size={14} /> Return to Supplier</button></div>}{formMode === 'return' ? <div className="grid md:grid-cols-2 gap-4"><Field label="Processed GRN to return from"><select className="input" value={returnGrnId} onChange={e => { const id=e.target.value; setReturnGrnId(id); const p=processed.find(x=>x.id===id); if(p) openReturn(p); else { setReturnPurchase(null); setReturnLines([]); } }}><option value="">Select processed GRN...</option>{processed.map(p => <option key={p.id} value={p.id}>{p.poNo} · {p.supplierName} · {fmtDate(p.date)}</option>)}</select></Field><Field label="Return reason *"><select className="input" value={returnReason} onChange={e => setReturnReason(e.target.value)}><option value="">Select reason...</option><option>Defective / Damaged</option><option>Wrong item received</option><option>Excess quantity</option><option>Quality issue</option><option>Price dispute</option><option>Other</option></select></Field></div>{formError && <div className="md:col-span-2 rounded-xl border border-rose-500/25 bg-rose-500/[0.06] px-4 py-3 text-sm text-rose-600">{formError}</div>}{returnPurchase && <div className="md:col-span-2 mt-5 space-y-2"><div className="text-xs font-bold uppercase tracking-wider text-sub mb-2">Items to return</div>{returnPurchase.items.map((item,itemIdx)=>{const line=returnLines.find(x=>x.itemIdx===itemIdx)||{itemIdx,maxQty:0,qty:0};return <div key={itemIdx} className="flex items-center gap-3 rounded-xl border border-line bg-surface p-3"><div className="min-w-0 flex-1"><div className="font-semibold text-ink truncate">{item.name}</div><div className="text-xs text-sub">Received: {item.qty} · Available: {line.maxQty} · Cost: {fmtRs(item.cost)}</div></div><input className="input num !w-24 text-center" type="number" min="0" max={line.maxQty} value={line.qty||''} placeholder="Qty" onChange={e=>{const qty=Math.max(0,Math.min(line.maxQty,Math.floor(Number(e.target.value)||0)));setReturnLines(ls=>ls.map(x=>x.itemIdx===itemIdx?{...x,qty}:x));}}/><div className="num w-24 text-right font-bold text-rose-600">{line.qty?fmtRs(line.qty*item.cost):'—'}</div></div>})}<div className="flex items-center justify-between rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 mt-4"><span className="font-semibold text-sub">Debit amount</span><span className="num text-lg font-extrabold text-rose-600">{fmtRs(returnLines.reduce((sum,line)=>sum+line.qty*(returnPurchase.items[line.itemIdx]?.cost||0),0))}</span></div><div className="flex justify-end mt-4"><button className="btn bg-rose-600 hover:bg-rose-700 text-white" disabled={!returnReason.trim()||!returnLines.some(x=>x.qty>0)} onClick={()=>{const result=createPurchaseReturn({purchaseId:returnPurchase.id,lines:returnLines.filter(x=>x.qty>0).map(x=>({itemIdx:x.itemIdx,qty:x.qty})),reason:returnReason.trim()});if(result){setReturnPurchase(null);setReturnLines([]);setReturnGrnId('');setReturnReason('');setFormError('');setView('list');}else{setFormError('Unable to create the supplier return. Check the GRN status, available stock, return quantity, and purchase permissions.');}}}}><Trash2 size={14}/> Create return & deduct stock</button></div></div>} : <div className="grid md:grid-cols-2 gap-4"><Field label="Supplier"><select className="input" value={supplierId} onChange={e => setSupplierId(e.target.value)}><option value="">Select supplier...</option>{state.suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field><Field label="Supplier invoice number"><input className="input" value={invoiceNo} onChange={e => setInvoiceNo(e.target.value)} placeholder="Optional" /></Field><Field label="Notes"><input className="input" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Optional receiving note" /></Field></div>}{formMode === 'receive' && <><div className="mt-5"><div className="flex items-center justify-between mb-2"><span className="text-xs font-bold uppercase tracking-wider text-sub">Items</span><button className="btn btn-soft !text-xs" onClick={() => setRows(r => [...r, emptyRow()])}><Plus size={13} /> Add line</button></div><div className="space-y-2">{rows.map((r, i) => <div key={i} className="grid grid-cols-[minmax(220px,1fr)_65px_100px_minmax(190px,1fr)_120px_36px] gap-2 items-center"><ProductSearchSelect value={r.productId} products={state.products} onChange={id => setRow(i, { productId: id })}/><input className="input num" type="number" min="1" value={r.qty || ''} onChange={e => setRow(i, { qty: Number(e.target.value) || 0 })} placeholder="Qty" /><input className="input num" type="number" min="0" step="0.01" value={r.cost || ''} onChange={e => setRow(i, { cost: Number(e.target.value) || 0 })} placeholder="Cost" />{(() => { const p=state.products.find(x=>x.id===r.productId); return p?.trackImei || p?.trackSerial ? <textarea className="input min-h-10 text-xs" rows={2} value={r.unitText} onChange={e=>setRow(i,{unitText:e.target.value})} placeholder={p.trackImei&&p.trackSerial ? 'IMEI,SERIAL — one unit per line' : p.trackImei ? 'IMEI — one per line' : 'SERIAL — one per line'} /> : <span className="text-xs text-faint px-2">Not tracked</span>; })()}<label className="flex items-center gap-2 text-xs text-sub"><input type="checkbox" checked={r.updateSellingPrice} onChange={e => setRow(i, { updateSellingPrice: e.target.checked })} /> Set sell price <span className="num">{r.sellingPrice.toLocaleString()}</span></label><button className="icon-btn !w-8 !h-8" disabled={rows.length === 1} onClick={() => setRows(rs => rs.filter((_, idx) => idx !== i))}><X size={14} /></button></div>)}</div></div><div className="flex items-center justify-between rounded-xl bg-violet-500/[0.06] border border-violet-500/15 px-4 py-3 mt-5"><span className="font-semibold text-sub">GRN Total</span><span className="num text-lg font-extrabold text-violet-500">{fmtRs(rows.reduce((s, r) => s + Math.max(0, r.qty) * Math.max(0, r.cost), 0))}</span></div>{formError && <div className="mt-4 rounded-xl border border-rose-500/25 bg-rose-500/[0.06] px-4 py-3 text-sm text-rose-600">{formError}</div>}<div className="flex gap-2.5 mt-5"><button className="btn btn-primary flex-1" onClick={saveDraft} disabled={!supplierId || !rows.some(r => r.productId && r.qty > 0)}><ClipboardCheck size={15} /> Save draft</button><button className="btn btn-outline-emerald flex-1" onClick={askProcess} disabled={!supplierId || !rows.some(r => r.productId && r.qty > 0)}><Zap size={15} /> Save & process</button></div></div></div>} : <div className="card overflow-hidden"><div className="p-4 border-b border-line"><div className="relative max-w-md"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-faint" /><input className="input pl-9" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search GRN, supplier or invoice..." /></div></div>{list.length === 0 ? <EmptyState icon={<ClipboardCheck size={26} />} title="No GRNs found" sub="Create a GRN draft to record incoming stock." /> : <div className="overflow-x-auto"><table className="w-full min-w-[900px]"><thead><tr><th className="th">GRN / PO</th><th className="th">Date</th><th className="th">Supplier</th><th className="th">Invoice</th><th className="th">Items</th><th className="th">Status</th><th className="th !text-right">Total</th><th className="th !text-right">Actions</th></tr></thead><tbody>{list.map(p => <tr key={p.id} className="hover:bg-raised/40"><td className="td font-bold text-violet-500">{p.poNo}</td><td className="td text-xs text-sub">{fmtDate(p.date)}</td><td className="td font-semibold">{p.supplierName}</td><td className="td text-xs text-sub">{p.supplierInvoiceNo || '—'}</td><td className="td text-xs text-sub">{p.items.map(i => `${i.name} ×${i.qty}`).join(', ')}</td><td className="td"><Badge tone={p.status === 'received' ? 'emerald' : 'amber'}>{p.status === 'received' ? <PackageCheck size={11} /> : <ClipboardCheck size={11} />}{p.status === 'received' ? 'PROCESSED' : 'DRAFT'}</Badge></td><td className="td num font-bold text-right">{fmtRs(p.total)}</td><td className="td"><div className="flex justify-end gap-1">{p.status === 'pending' && <button className="icon-btn !w-8 !h-8" title="Edit" onClick={() => edit(p)}><Pencil size={14} /></button>}<button className="icon-btn !w-8 !h-8" title="Print" onClick={() => { setPrintReady(false); setPrintPreview({ grn: p, html: grnPrintHtml(p, state.settings.shopName) }); }}><Printer size={14} /></button>{p.status === 'pending' && <button className="btn btn-outline-emerald !py-1.5 !px-2.5 !text-xs" onClick={() => {
  if (!validatePurchaseUnitIdentifiers(p, state.products, state.units || [])) {
    setFormError('This GRN has missing, duplicated, or invalid IMEI/Serial values. Edit the draft and correct them before processing.');
...
}}><Zap size={13} /> Process</button>}{p.status === 'pending' && can('act:deleteRecords') && <button className="icon-btn !w-8 !h-8 hover:!text-rose-500" onClick={() => setDeleteId(p.id)}><Trash2 size={14} /></button>}</div></td></tr>)}</tbody></table></div>}</div>}
