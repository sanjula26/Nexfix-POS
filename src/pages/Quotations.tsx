  ...
  {r.status !== 'converted' && r.status !== 'rejected' && <button type="button" className="btn btn-soft !py-1.5 !px-3 text-xs ml-2" onClick={() => convertToSale(r)}><ShoppingCart size={14} /> To Sale</button>}
    ...
    <Modal open={!!editing} onClose={() => { setEditing(null); setMsg(''); }} title={isNew ? 'New Quotation' : editing?.quoteNo || 'Quote'} sub={editing?.customerName} wide>
      {editing && <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3"><Field label="Customer name"><input className="input" value={editing.customerName} onChange={e => setEditing({ ...editing, customerName: e.target.value })} /></Field><Field label="Phone"><input className="input" value={editing.customerPhone || ''} onChange={e => setEditing({ ...editing, customerPhone: e.target.value })} /></Field><Field label="Validity (days)" hint="Optional"><input type="number" min="1" className="input num" value={editing.validUntil ? Math.max(1, Math.ceil((new Date(editing.validUntil).getTime() - new Date(editing.createdAt).getTime()) / 86400000)) : ''} onChange={e => { const days = Math.max(1, Number(e.target.value) || 0); setEditing({ ...editing, validUntil: days ? new Date(new Date(editing.createdAt).getTime() + days * 86400000).toISOString() : undefined }); }} /></Field><Field label="Valid until"><input type="date" className="input" value={editing.validUntil ? editing.validUntil.slice(0, 10) : ''} onChange={e => setEditing({ ...editing, validUntil: e.target.value ? new Date(e.target.value + "T23:59:59").toISOString() : undefined })} /></Field></div></div>
