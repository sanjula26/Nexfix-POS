    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 items-start">
        ...
        <div className="space-y-5">
          ...
          <div className="card p-6"><h3 className="font-bold text-ink flex items-center gap-2 mb-2"><span className="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-600 flex items-center justify-center"><Cloud size={15} /></span>Google Drive Backup</h3><p className="text-xs text-faint mb-3">Automatic Google Drive backup is centrally managed. The POS sends each shop's backup to its own isolated folder; shop users do not need to configure a Google URL.</p><div className="flex items-center gap-2 text-[12px] font-semibold text-emerald-600 mb-3"><CheckCircle2 size={15} /> Automatic backup is enabled</div><div className="flex flex-wrap gap-2 mt-3">{user?.role === 'admin' && <><button type="button" className="btn btn-primary" disabled={!gEnabled || connectivity !== 'online' || gRestoreBusy} onClick={runGoogleBackupNow}><Cloud size={15} /> {gRestoreBusy ? 'Backing up…' : 'Backup now to Google Drive'}</button><button type="button" className="btn btn-soft" disabled={!gEnabled || connectivity !== 'online' || gRestoreBusy} onClick={requestGoogleRestore}><RotateCcw size={15} /> {gRestoreBusy ? 'Working…' : 'Restore latest Google backup'}</button></>}</div>{gMsg && <p className={`text-[13px] font-medium mt-3 ${gMsg.includes('Failed') || gMsg.includes('disabled') || gMsg.includes('requires') || gMsg.includes('No valid') || gMsg.includes('Invalid') || gMsg.includes('Could not') || gMsg.includes('failed') ? 'text-rose-500' : 'text-emerald-500'}`}>{gMsg}</p>}<p className="text-[11px] text-faint mt-3">If Windows is reinstalled or this POS is moved to a new PC, import a downloaded Google Drive backup and the encrypted backup's Shop Backup ID is adopted automatically after validation. You can also enter the existing Shop Backup ID above before using cloud restore.</p></div>
