    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 items-start">
        ...
        <div className="space-y-5">
          ...
          <div className="card p-6"><h3 className="font-bold text-ink flex items-center gap-2 mb-2"><span className="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-600 flex items-center justify-center"><Cloud size={15} /></span>Google Drive Sync &amp; Cloud Backup</h3><p className="text-xs text-faint mb-3">Linked to your Google account via Google Apps Script. Each shop gets its own Google Drive backup folder, and full POS state is backed up automatically when the configured interval is due.</p><div className="flex items-center justify-between gap-3 mb-3"><span className="text-sm font-medium text-ink">Enable Google sync</span><Toggle checked={gEnabled} onChange={v => { if (!can('act:export')) { setGMsg('Google sync requires export permission'); return; } setGEnabled(v); setGoogleSyncEnabled(v); setGMsg(v ? 'Google sync enabled' : 'Google sync disabled'); }} /></div><Field label="Apps Script Web App URL" hint="Deploy as web app → Anyone → copy URL"><input className="w-full px-3 py-2 rounded-lg border border-line bg-raised text-sm" value={gUrl} onChange={e => setGUrl(e.target.value)} placeholder="https://script.google.com/macros/s/…/exec" /></Field><div className="flex flex-wrap gap-2 mt-3"><button type="button" className="btn btn-soft" onClick={() => { try { setGoogleScriptUrl(gUrl); const enabled = !!gUrl.trim(); if (enabled) { setGoogleSyncEnabled(true); setGEnabled(true); } setGMsg(enabled ? 'Google Drive backup configured and auto-backup enabled' : 'Google Script URL cleared'); } catch (error) { setGMsg(error instanceof Error ? error.message : 'Invalid Google Script URL'); } }}>Save URL</button>{can('act:export') && <button type="button" className="btn btn-emerald" disabled={!gEnabled || connectivity !== 'online'} onClick={async () => { setGMsg('Sending full backup to Google…'); const result = await downloadBackup(state, 'manual', { download: false, cloud: true });
          setGMsg(result.cloud ? 'Full backup sent to Google Drive' : 'Failed (offline or script error)'); }}><Cloud size={15} /> Backup now to Google</button>}{user?.role === 'admin' && <button type="button" className="btn btn-soft" disabled={!gEnabled || connectivity !== 'online' || gRestoreBusy} onClick={requestGoogleRestore}><RotateCcw size={15} /> {gRestoreBusy ? 'Reading backup…' : 'Restore latest Google backup'}</button>}</div>{gMsg && <p className={`text-[13px] font-medium mt-3 ${gMsg.includes('Failed') || gMsg.includes('disabled') || gMsg.includes('requires') || gMsg.includes('No valid') || gMsg.includes('Invalid') || gMsg.includes('Could not') || gMsg.includes('failed') ? 'text-rose-500' : 'text-emerald-500'}`}>{gMsg}</p>}<p className="text-[11px] text-faint mt-3">Restore reads the newest shop-specific Google Drive backup snapshot, validates it, creates a local safety checkpoint, then replaces local data only after confirmation.</p></div>
