...
const features = [
  ...
  { icon: RefreshCw, tint: 'from-violet-500 to-purple-600', title: 'Offline Mode', sub: 'Auto sync queue' },
];
    ...
    <div className="min-h-screen flex bg-[#f5f6fb]">
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative max-w-lg">
          ...
          <p className="mt-4 text-[15px] text-[#a9add4] font-medium">
            POS &amp; Inventory Management System <span className="text-[#6e72a2]">&middot; v3.1</span>
          </p>
          ...
          <div className="grid grid-cols-2 gap-3.5 mt-10">
            {features.map((f, i) => (
              <motion.div
                ...
                className="rounded-2xl bg-white/[0.05] border border-white/[0.09] p-4 backdrop-blur-sm hover:bg-white/[0.09] transition-colors"
              >
                <span className={`inline-flex w-9 h-9 rounded-xl bg-gradient-to-br ${f.tint} items-center justify-center shadow-lg mb-3`}>
          ...
          <div className="mt-12 pt-6 border-t border-white/10 text-xs text-[#7f83ad] font-medium">
            Nexfix Solution &middot; +94 74 109 7350 &middot; info@nexfixsolution.com
          </div>
        </motion.div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative">
        ...
        <div className="w-full max-w-md">
          <Link
            to="/signup"
            className="inline-flex items-center gap-2 text-xs font-semibold text-violet-600 bg-violet-500/10 border border-violet-500/20 rounded-full px-4 py-2 mb-5 hover:bg-violet-500/15 transition-colors"
          >
          ...
          <motion.div
            ...
            className="bg-white rounded-3xl shadow-[0_24px_70px_-24px_rgba(23,19,60,0.35)] border border-[#eceef6] overflow-hidden"
          >
            <div className="h-1.5 bg-gradient-to-r from-violet-600 via-indigo-500 to-sky-400" />
            <div className="p-7 sm:p-9">
              <h2 className="font-display text-[26px] font-extrabold text-[#17133c] flex items-center gap-2">
                Welcome back
                <span className="inline-flex w-7 h-7 rounded-lg bg-amber-100 items-center justify-center">
                  <UserRound size={15} className="text-amber-600" />
                </span>
              </h2>
              ...
              <form onSubmit={submit} className="mt-7 space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9a9ebf]" />
                    <input
                      className="input !bg-[#f5f6fb] !border-[#e7e9f2] pl-9 !py-3"
                ...
                <div>
                  ...
                  <div className="relative">
                    <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9a9ebf]" />
                    <input
                      className="input !bg-[#f5f6fb] !border-[#e7e9f2] pl-9 pr-10 !py-3"
                      placeholder="•••••••••••"
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9a9ebf] hover:text-[#5b5f7e]"
                    >
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <span
                      ...
                      className={`w-[18px] h-[18px] rounded-[5px] flex items-center justify-center border transition-all ${
                        remember ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-[#d4d7ea]'
                      }`}
                    >
                ...
                {error && (
                  <motion.div
                    ...
                    className="flex items-center gap-2 text-[13px] font-medium text-rose-600 bg-rose-50 border border-rose-200 rounded-xl px-3.5 py-2.5"
                  >
                ...
                >
                  {loading ? <Loader2 size={17} className="animate-spin" /> : <>Sign in &amp; stay logged in <ArrowRight size={16} /></>}
                </button>
              </form>
              ...
              <p className="text-center text-[13px] text-[#5b5f7e] mt-6">
                Don't have an account?{' '}
                <Link to="/signup" className="font-bold text-violet-600 hover:text-violet-700">Sign up</Link>
              </p>
            </div>
          </motion.div>
        </div>
