...
function BrandingHero() {
  return (
    <section className="relative flex min-h-[360px] w-full flex-1 flex-col justify-center overflow-hidden px-6 py-10 sm:px-10 lg:min-h-screen lg:px-12 xl:px-16">
      ...
      />
      <div aria-hidden="true" className="pointer-events-none absolute -left-24 top-1/4 h-72 w-72 rounded-full bg-cyan-500/15 blur-3xl" />
      <div aria-hidden="true" className="pointer-events-none absolute bottom-0 right-0 h-80 w-80 rounded-full bg-indigo-500/15 blur-3xl" />

      <div className="relative z-10 mx-auto w-full max-w-2xl">
        <div className="mb-5 inline-flex items-center rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3.5 py-2 text-[11px] font-bold tracking-[0.08em] text-cyan-200 shadow-[0_0_24px_rgba(34,211,238,0.12)]">
          Sri Lanka's Trusted Retail POS
        </div>

        <div className="flex items-center gap-4">
          <div className="relative grid h-16 w-16 shrink-0 place-items-center rounded-2xl border border-cyan-300/50 bg-gradient-to-br from-cyan-300 via-sky-500 to-indigo-600 shadow-[0_0_38px_rgba(34,211,238,0.35)]">
          ...
          <div>
            <h1 className="text-[clamp(2.4rem,5vw,5.2rem)] font-black leading-[0.9] tracking-[-0.055em]">
              <span className="text-white">NEXFIX</span>{' '}
            ...
            <p className="mt-3 text-sm font-medium tracking-wide text-slate-300 sm:text-base">
              POS &amp; Inventory Management System · v3.0.2
            </p>
          </div>
        </div>
        ...
        <p className="mt-8 text-xs font-medium tracking-wide text-slate-400">
          Nexfix Solution ·{' '}
          <a className="transition hover:text-cyan-300" href="tel:+94741097350">+94 74 109 7350</a>
          {' · '}
...
function FeatureCard({
  ...
  const tones = {
    lime: 'border-lime-300/45 bg-lime-300/[0.05] text-lime-200 shadow-[0_0_24px_rgba(190,242,100,0.09)]',
    pink: 'border-fuchsia-300/45 bg-fuchsia-300/[0.05] text-fuchsia-200 shadow-[0_0_24px_rgba(232,121,249,0.09)]',
    green: 'border-emerald-300/45 bg-emerald-300/[0.05] text-emerald-200 shadow-[0_0_24px_rgba(52,211,153,0.09)]',
    cyan: 'border-cyan-300/45 bg-cyan-300/[0.05] text-cyan-200 shadow-[0_0_24px_rgba(34,211,238,0.09)]',
  };
  return (
    <div className={`rounded-2xl border px-4 py-4 backdrop-blur-sm transition duration-200 hover:-translate-y-0.5 ${tones[tone]}`}>
      <div className="flex items-center gap-3">
        <div className="grid h-9 w-9 place-items-center rounded-xl bg-black/20">{icon}</div>
        <div>
  ...
  return (
    <section className="relative flex w-full items-center justify-center px-5 py-8 sm:px-8 lg:min-h-screen lg:px-10">
      <div className="pointer-events-none absolute inset-0 hidden lg:block">
        <div className="absolute right-[-8rem] top-[12%] h-72 w-72 rounded-full bg-cyan-500/10 blur-3xl" />
        <div className="absolute bottom-[10%] left-[15%] h-64 w-64 rounded-full bg-indigo-500/10 blur-3xl" />
        <div className="absolute right-[18%] top-[24%] h-2 w-2 rounded-full bg-cyan-300 shadow-[0_0_18px_5px_rgba(103,232,249,0.35)]" />
        <div className="absolute right-[30%] bottom-[24%] h-1.5 w-1.5 rounded-full bg-sky-300 shadow-[0_0_15px_4px_rgba(125,211,252,0.3)]" />
      </div>
      <div className={`relative w-full max-w-[460px] overflow-hidden rounded-[28px] border border-cyan-300/20 bg-[#071225]/90 shadow-[0_25px_80px_rgba(2,8,23,0.55)] backdrop-blur-xl ${setup ? 'my-2' : 'lg:my-10'}`}>
...
export default function Login() {
  ...
  const page = (
    <main className="min-h-screen overflow-x-hidden bg-[#030a18] text-slate-100">
        ...
        {state.users.length === 0 ? (
          <AuthCard setup>
            <div className="p-6 sm:p-8">
              <div className="mb-5 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-black tracking-tight text-white">Set up your administrator</h2>
                  <p className="mt-1 text-xs text-slate-400">Create the first secure POS accounts.</p>
                </div>
              </div>
              <form onSubmit={setupAdmin} className="space-y-3.5">
                <input required value={setupName} onChange={e => setSetupName(e.target.value)} className="input w-full !rounded-xl !border-white/10 !bg-[#0b1930] !py-3 !text-white placeholder:!text-slate-500 focus:!border-cyan-400/60" placeholder="Administrator name" autoComplete="name" />
                <input required type="text" inputMode="email" value={setupEmail} onChange={e => setSetupEmail(e.target.value)} className="input w-full !rounded-xl !border-white/10 !bg-[#0b1930] !py-3 !text-white placeholder:!text-slate-500 focus:!border-cyan-400/60" placeholder="Administrator email" autoComplete="username" autoCapitalize="none" autoCorrect="off" spellCheck={false} />
                ...
                <div className="my-2 border-t border-white/10 pt-3">
                  <p className="mb-2 text-[10px] font-bold tracking-[0.18em] text-cyan-300/80">CASHIER ACCOUNT</p>
          ...
          <AuthCard>
            <div className="p-7 sm:p-9 lg:p-10">
              ...
              <div className="mt-7 grid grid-cols-2 gap-1.5 rounded-xl border border-white/10 bg-[#0b1930] p-1.5">
                <button type="button" onClick={() => { setLoginRole('admin'); setError(''); }} className={`rounded-lg py-3 text-xs font-black tracking-wider transition ${loginRole === 'admin' ? 'bg-gradient-to-r from-sky-400 to-blue-600 text-white shadow-[0_6px_20px_rgba(59,130,246,0.28)]' : 'text-slate-400 hover:text-white'}`}>ADMIN</button>
              ...
              <form onSubmit={submit} className="mt-7 space-y-5">
                <label className="block">
                  <span className="mb-2 block text-[10px] font-bold tracking-[0.2em] text-slate-400">EMAIL</span>
                  <div className="relative">
                    <Mail size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" />
                    <input required type="text" inputMode="email" autoComplete="username" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={email} onChange={e => setEmail(e.target.value)} className="input w-full !rounded-xl !border-white/10 !bg-[#0b1930] !py-3.5 !pl-10 !text-white placeholder:!text-slate-600 focus:!border-cyan-400/70 focus:!ring-cyan-400/20" placeholder="you@shop.lk" />
                  </div>
                </label>
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    ...
                    <input required type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} className="input w-full !rounded-xl !border-white/10 !bg-[#0b1930] !py-3.5 !pl-10 !pr-11 !text-white placeholder:!text-slate-600 focus:!border-cyan-400/70 focus:!ring-cyan-400/20" placeholder="Password" />
                ...
                <label className="flex cursor-pointer items-center gap-2.5 text-xs font-medium text-slate-400 select-none">
                  <input
                    type="checkbox"
                    checked={remember}
                    onChange={e => handleRememberChange(e.target.checked)}
                    className="h-4 w-4 rounded border-slate-600 bg-[#0b1930] text-cyan-400 accent-cyan-400"
                  />
...
function ErrorAlert({ message }: { message: string }) {
  return (
    <div role="alert" className="flex items-start gap-2.5 rounded-xl border border-rose-400/25 bg-rose-500/10 px-3.5 py-3 text-[12px] font-medium text-rose-200">
