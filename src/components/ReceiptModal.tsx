    ...
    >
      <div className={`${forPrint ? 'w-[72mm]' : 'w-full'} bg-white text-black p-5 text-[12px] leading-relaxed`}>
        ...
        <div className="space-y-0.5 text-[11px]">
          ...
          {sale.discount > 0 && <div className="flex justify-between"><span>Discount</span><span>- {fmtRs(sale.discount, false)}</span></div>}
          {sale.tax > 0 && <div className="flex justify-between"><span>Tax</span><span>{fmtRs(sale.tax, false)}</span></div>}
...
export default function ReceiptModal({ sale, onClose, onNewSale }: {
  ...
  const waUrl = customer?.phone ? waLink(customer.phone, buildWhatsAppText(sale, state.settings)) : '';
  return (
    <>
