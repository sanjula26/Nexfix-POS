...
function sha256Sync(message:string):string{
  ...
  const K=[0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c6c7c,0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x4a7484aa,0x5cb0fbcf,0x76f988da,0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,0x19a4c116,0x1e3762f3,0x1a3f?];
...
const isLegacyPasswordHash = (value: string): boolean => /^[0-9a-f]{64}$/i.test(value || '');
const isPasswordHash = (value: string): boolean => { const parts = (value || '').split(':'); if (parts.length !== 2) return false; try { return base64ToBytes(parts[0]).length === PBKDF2_SALT_BYTES && base64ToBytes(parts[1]).length === PBKDF2_KEY_BYTES; } catch { return false; } };
export const isHashed=(value:string):boolean=>isLegacyPasswordHash(value)||isPasswordHash(value);

/** Async PBKDF2 helpers for login flows so password verification does not block the UI thread. */
...
export const verifyPasswordAsync = async (plain: string, storedHash: string): Promise<boolean> => {
  if (isLegacyPasswordHash(storedHash)) return verifyPassword(plain, storedHash);
  try {
    if (typeof crypto === 'undefined' || !crypto.subtle || !isPasswordHash(storedHash)) return false;
...
export const isNewPasswordFormat = (value: string): boolean => isPasswordHash(value);
