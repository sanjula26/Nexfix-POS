...
const SHA256_K=[0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c6c7c,0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x4a7484aa,0x5cb0a9dc,0x76f988da,0x983e5152,0xa831c66d,0xb00327c8,0xc6e00bf3,0xd192e819,0x106aa070,0x19a4c116,0x2748774c,0x34b0bcb5,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xc67178f2];
const rotr=(x:number,n:number)=>(x>>>n)|(x<<(32-n));
const sha256Bytes=(input:Uint8Array):Uint8Array=>{const bitLen=input.length*8;const total=((input.length+9+63)>>6)*64;const data=new Uint8Array(total);data.set(input);data[input.length]=0x80;const view=new DataView(data.buffer);view.setUint32(total-4,bitLen>>>0,false);view.setUint32(total-8,Math.floor(bitLen/0x100000000)>>>0,false);let h0=0x6a09e667,h1=0xbb67ae85,h2=0x3c6ef372,h3=0xa54ff53a,h4=0x510e527f,h5=0x9b05688c,h6=0x1f83d9ab,h7=0x5be0cd19;const w=new Uint32Array(64);for(let off=0;off<total;off+=64){for(let i=0;i<16;i++)w[i]=view.getUint32(off+i*4,false);for(let i=16;i<64;i++){const s0=rotr(w[i-15],7)^rotr(w[i-15],18)^(w[i-15]>>>3);const s1=rotr(w[i-2],17)^rotr(w[i-2],19)^(w[i-2]>>>10);w[i]=(w[i-16]+s0+w[i-7]+s1)>>>0;}let a=h0,b=h1,c=h2,d=h3,e=h4,f=h5,g=h6,h=h7;for(let i=0;i<64;i++){const S1=rotr(e,6)^rotr(e,11)^rotr(e,25);const ch=(e&f)^(~e&g);const t1=(h+S1+ch+SHA256_K[i]+w[i])>>>0;const S0=rotr(a,2)^rotr(a,13)^rotr(a,22);const maj=(a&b)^(a&c)^(b&c);const t2=(S0+maj)>>>0;h=g;g=f;f=e;e=(d+t1)>>>0;d=c;c=b;b=a;a=(t1+t2)>>>0;}h0=(h0+a)>>>0;h1=(h1+b)>>>0;h2=(h2+c)>>>0;h3=(h3+d)>>>0;h4=(h4+e)>>>0;h5=(h5+f)>>>0;h6=(h6+g)>>>0;h7=(h7+h)>>>0;}const out=new Uint8Array(32);const vals=[h0,h1,h2,h3,h4,h5,h6,h7];for(let i=0;i<8;i++)out.set([(vals[i]>>>24)&255,(vals[i]>>>16)&255,(vals[i]>>>8)&255,vals[i]&255],i*4);return out;};
...
export const verifyPassword=(plain:string,storedHash:string):boolean=>{if(/^[0-9a-f]{64}$/i.test(storedHash||''))return verifyLegacyPassword(plain,storedHash);try{const parts=storedHash.split(':');if(parts.length!==2)return false;const salt=base64ToBytes(parts[0]);const expected=base64ToBytes(parts[1]);if(salt.length!==PBKDF2_SALT_BYTES||expected.length!==PBKDF2_KEY_BYTES)return false;return constantTimeEqual(expected,pbkdf2Sha256(plain,salt,PBKDF2_ITERATIONS,PBKDF2_KEY_BYTES));}catch{return false;}};
