...
export async function idbAcknowledgeQueue(ids:string[]):Promise<void>{if(!ids.length)return;try{const db=await openDB();const store=db.transaction(STORE_QUEUE,'readwrite').objectStore(STORE_QUEUE);for(const id of ids)await idbReq(store.delete(id));db.close();}catch{}}
...
export async function idbSetMeta(patch:Partial<BackupMeta>):Promise<BackupMeta>{const current=await idbGetMeta();const next={...current,...patch};try{const db=await openDB();await idbReq(db.transaction(STORE_META,'readwrite').objectStore(STORE_META).put({key:'backup',value:next}));db.close();}catch{}return next;}
