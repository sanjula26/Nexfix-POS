...
export async function idbAcknowledgeQueue(ids:string[]):Promise<void>{if(!ids.length)return;try{const db=await getDB();const store=db.transaction(STORE_QUEUE,'readwrite').objectStore(STORE_QUEUE);for(const id of ids)await idbReq(store.delete(id));}catch{/* best-effort cleanup */}}
