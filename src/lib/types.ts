...
export type PaymentLegAmount = number;
...
export interface Exchange { id: string; date: string; saleId?: string; fromProductId?: string; toProductId?: string; note?: string; by: string; }
export interface AuditEntry { id: string; date: string; action: string; entity: string; details: string; by: string; }
export interface HeldSale { id: string; createdAt: string; by: string; items: SaleItem[]; }
export interface DaySession { cashierId: string; date: string; opening: number; closing?: number; closed: boolean; note?: string; }
export interface Quotation { id: string; quoteNo: string; date: string; customerName: string; items: SaleItem[]; subtotal: number; discount: number; tax: number; total: number; notes?: string; convertedSaleId?: string; createdAt: string; by: string; }
...
export interface RepairJob { id: string; jobNo: string; customerId?: string; customerName: string; phone?: string; device?: string; imeiOrSerial?: string; issue?: string; status?: string; createdAt: string; updatedAt?: string; by: string; }
...
/** Permission map is extensible so every role can have explicit permissions. */
