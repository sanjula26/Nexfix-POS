...
export interface Product {
  id: string; name: string; sku: string; barcode: string; category: string; brand: string;
  cost: number; price: number; stock: number; reorderLevel: number; trackImei: boolean;
  trackSerial?: boolean; trackExpiry?: boolean; warrantyMonths?: number; isKit?: boolean;
...
export interface InventoryUnit {
  id: string; productId: string; imei?: string; serial?: string; expiryDate?: string; status: UnitStatus;
  purchaseId?: string; saleId?: string; saleBillNo?: string; cost?: number; note?: string; createdAt: string;
  soldAt?: string; warrantyExpiresAt?: string;
}
export interface Customer {
  id: string; name: string; phone: string; email?: string; nic?: string; address?: string; createdAt: string;
  creditBalance: number; loyaltyPoints: number;
}
...
export interface SaleItem {
  productId: string; name: string; qty: number; price: number; cost: number; discount?: number;
  priceOverridden?: boolean; unitIds?: string[]; imeis?: string[]; serials?: string[]; warrantyMonths?: number;
}
...
export interface Sale {
  ...
  /** Persistent POS terminal identifier used for multi-machine sales/income reporting. */
  machineId: string;
  /** Human-readable terminal label captured at sale time. */
  machineName: string;
  customerId?: string; customerName: string; items: SaleItem[]; subtotal: number; discount: number; tax: number;
  total: number; payment: PaymentMethod; payments?: PaymentLeg[]; shipping?: number; pointsRedeemed?: number;
  pointsEarned?: number; note?: string; amountPaid: number; change: number; profit: number;
...
export interface RepairJob {
  ...
  receivedAt: string; promisedAt?: string; completedAt?: string; deliveredAt?: string; technicianId?: string;
...
export interface Quotation {
  id: string; quoteNo: string; customerId?: string; customerName: string; customerPhone?: string; status: QuoteStatus;
  items: QuotationItem[]; subtotal: number; discount: number; tax: number; total: number; validUntil?: string;
  notes?: string; convertedSaleId?: string; createdAt: string; by: string;
}
export type ClaimStatus = 'open' | 'approved' | 'rejected' | 'replaced' | 'repaired' | 'closed';
export interface WarrantyClaim {
  id: string; claimNo: string; unitId?: string; saleId?: string; saleBillNo?: string; customerId?: string;
  customerName?: string; productName: string; imeiOrSerial?: string; issueDescription: string; status: ClaimStatus;
  resolutionNotes?: string; createdAt: string; closedAt?: string; by: string;
}
