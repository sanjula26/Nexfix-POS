...
async function authorize(req: Request) {
  ...
  if (!memberships?.some((membership) => ALLOWED_ROLES.has(String(membership.role)))) {
    return { ok: false as const, status: 403, message: 'Google backup access requires an active admin or manager membership' };
  }
