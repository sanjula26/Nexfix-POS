create or replace function private.list_sale_reversal_requests(p_shop_id uuid)
returns setof jsonb language sql security definer set search_path to '' as $$
 ...
 from public.sale_reversal_requests r
...
returns setof jsonb language sql security invoker set search_path to '' as $$ select * from private.list_sale_reversal_requests($1) $$;
revoke all on function private.list_sale_reversal_requests(uuid) from public,anon,authenticated;
