...
declare
  ...
  v_product_id uuid;
  v_qty numeric;
  v_requested_units uuid[];
  v_available_units uuid[];
  v_row record;
  v_available numeric;
begin
  ...
  loop
    ...
    v_requested_units := case when jsonb_typeof(v_line->'unit_ids') = 'array' then array(select x::uuid from jsonb_array_elements_text(v_line->'unit_ids') x) else '{}'::uuid[] end;
    if v_product_id is null or v_qty is null or v_qty <= 0 then raise exception 'Invalid return line'; end if;

    v_row := null;
    if cardinality(v_requested_units) > 0 then
      select si.id, si.product_id, si.qty, si.unit_ids into v_row
      from public.sale_items si
      where si.sale_id = p_sale_id and si.product_id = v_product_id
        and exists (select 1 from unnest(si.unit_ids) u where u = any(v_requested_units))
      order by si.id limit 1;
    else
      select si.id, si.product_id, si.qty, si.unit_ids into v_row
      from public.sale_items si where si.sale_id = p_sale_id and si.product_id = v_product_id
      order by si.id limit 1;
    end if;
    ...
    if cardinality(v_requested_units) > 0 then
      select coalesce(array_agg(u.id order by array_position(v_requested_units,u.id)), '{}'::uuid[]) into v_available_units
      from public.inventory_units u
      where u.shop_id = p_shop_id and u.product_id = v_product_id and u.sale_id = p_sale_id and u.status = 'sold'
        and u.id = any(v_requested_units);
      if cardinality(v_available_units) < v_qty then raise exception 'Tracked unit availability changed'; end if;
      v_available_units := v_available_units[1:floor(v_qty)::int];
    else
      v_available_units := '{}'::uuid[];
    end if;
    ...
    v_result := v_result || jsonb_build_array(jsonb_build_object(
      ...
      'qty', v_qty,
      'unit_ids', to_jsonb(v_available_units)
    ));
  end loop;
  return v_result;
end;
