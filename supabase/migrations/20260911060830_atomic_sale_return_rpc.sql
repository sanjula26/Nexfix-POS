-- Atomic return/refund RPC. The implementation is private SECURITY DEFINER;
-- the public entry point is an authenticated SECURITY INVOKER wrapper.
...
declare
  v_uid uuid:=auth.uid(); v_role text; v_sale public.sales%rowtype; v_existing public.sale_returns%rowtype; v_return_no text;
  v_refund numeric:=0; v_additional numeric:=0; v_sale_subtotal numeric:=0; v_sale_discount numeric:=0; v_post_discount numeric:=0; v_tax_refund numeric:=0; v_shipping_refund numeric:=0; v_returned_merch numeric:=0; v_allocated_discount numeric:=0; v_taxable_returned numeric:=0;
  v_full_return boolean:=false; v_returned_ratio numeric:=0; v_points_reverse integer:=0; v_points_restore integer:=0; v_credit_due numeric:=0; v_credit_reduction numeric:=0;
  v_line jsonb; v_sale_item public.sale_items%rowtype; v_qty numeric; v_prior numeric; v_available numeric; v_amount numeric; v_unit_ids uuid[]; v_id uuid; v_remaining numeric; v_remaining_lines integer:=0; v_total_lines integer:=0;
begin
  ...
  if found then if v_existing.sale_id<>p_sale_id then raise exception 'Return id already belongs to another sale'; end if; return jsonb_build_object('ok',true,'already_committed',true,'return_id',v_existing.id,'return_no',v_existing.return_no,'refund_amount',v_existing.refund_amount,'additional_payment',v_existing.additional_payment); end if;
  ...
  select count(*) into v_total_lines from public.sale_items where sale_id=v_sale.id; if v_total_lines=0 then raise exception 'Sale has no items'; end if;
  for v_line in select * from jsonb_array_elements(p_lines) loop
    v_id:=(v_line->>'sale_item_id')::uuid; v_qty:=(v_line->>'qty')::numeric; if v_qty is null or v_qty<=0 then raise exception 'Invalid return quantity'; end if;
    select * into v_sale_item from public.sale_items where id=v_id and sale_id=v_sale.id for update; if not found then raise exception 'Sale item not found'; end if;
    ...
    if coalesce(array_length(v_sale_item.unit_ids,1),0)>0 then
      ...
      foreach v_id in array v_unit_ids loop perform 1 from public.inventory_units where id=v_id and shop_id=p_shop_id and product_id=v_sale_item.product_id and status='sold' and sale_id=v_sale.id for update; if not found then raise exception 'Tracked unit is not currently sold on this sale'; end if; end loop;
  ...
  v_sale_subtotal:=greatest(0,coalesce(v_sale.subtotal,0)); v_sale_discount:=least(greatest(0,coalesce(v_sale.discount,0)),v_sale_subtotal); v_post_discount:=greatest(0,v_sale_subtotal-v_sale_discount);
  v_allocated_discount:=least(v_sale_discount,v_returned_merch*case when v_sale_subtotal>0 then v_sale_discount/v_sale_subtotal else 0 end); v_taxable_returned:=greatest(0,v_returned_merch-v_allocated_discount);
  ...
  v_refund:=greatest(0,round((v_taxable_returned+v_tax_refund+v_shipping_refund)::numeric,2)); if p_mode='replace' then v_refund:=0; v_additional:=0; end if;
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    v_id:=(v_line->>'sale_item_id')::uuid; v_qty:=(v_line->>'qty')::numeric; select * into v_sale_item from public.sale_items where id=v_id and sale_id=v_sale.id;
  ...
  if v_sale.customer_id is not null then
    v_credit_due:=greatest(0,coalesce(v_sale.total,0)-coalesce(v_sale.amount_paid,0)); v_credit_reduction:=least(v_credit_due,v_refund); v_returned_ratio:=case when coalesce(v_sale.total,0)>0 then least(1,v_refund/v_sale.total) else 1 end;
    v_points_reverse:=least(coalesce(v_sale.points_earned,0),round(coalesce(v_sale.points_earned,0)*v_returned_ratio)); v_points_restore:=least(coalesce(v_sale.points_redeemed,0),round(coalesce(v_sale.points_redeemed,0)*v_returned_ratio));
...
returns jsonb language sql security invoker set search_path to '' as $wrapper$ select private.process_sale_return_atomic($1,$2,$3,$4,$5,$6,$7) $wrapper$;
