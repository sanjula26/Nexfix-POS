-- Nexfix POS: bind cloud snapshot writes to a registered device.
-- A device identifier is only an identifier, not a secret; authentication and
-- owner checks remain the security boundary.
...
begin
  ...
  -- The caller must have explicitly registered this device for this shop.
  if not exists (
    select 1
  ...
  if not found then
    ...
    values
      (normalized_shop, auth.uid(), p_state, 1, now(), auth.uid());
