-- Phase 1 / Task 2: server-side atomic sale transaction.
-- Keeps the existing local-first UI intact while providing a trusted
-- transaction boundary for online/multi-PC sales.
...
begin
  ...
  v_total := greatest(0, round(
    ...
    - least(v_points_value, v_subtotal - v_discount + v_tax + v_shipping), 2
  ));
  v_points_earned := case when p_customer_id is null then 0 else floor(v_total / v_earn_div)::integer end;
  v_profit := round(v_profit - v_discount - v_points_value + v_shipping, 2);
...
comment on function public.complete_sale_atomic(uuid, uuid, uuid, numeric, numeric, numeric, integer, text, uuid, jsonb, jsonb)
is 'Trusted atomic POS sale: validates shop membership, locks stock/counter/customer/unit rows, inserts sale/items/payments, updates inventory/credit/loyalty, and rolls back the whole transaction on failure.';
